...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {sidebarVisible && (
        <aside
          className="fixed left-1/2 top-20 z-60 w-66 -translate-x-1/2 rounded-lg bg-background-secondary md:hidden"
          onClick={(e) => e.stopPropagation()}
        >
          <nav className="pt-4 pr-6 pb-4">
              ...
              {boards.map((board, index) => (
                <li key={index}>
                  <NavLink
                    ...
                    onClick={toggleSidebar}
                    className={({ isActive }) => `
                      flex items-center gap-4 rounded-r-full py-4 pl-6 transition-all
                      ${
                        isActive
      ...
      <div
        className={`
        fixed top-0 left-0 z-50 h-24 hidden md:flex items-center px-8 
      ...
      <aside
        className={`
          fixed top-24 left-0 z-40 h-[calc(100vh-6rem)] hidden md:flex bg-background-secondary dark:bg-dark-bg-secondary 
          border-r border-border dark:border-dark-border transition-all duration-300 flex-col overflow-hidden
          ${sidebarVisible ? "w-65.25 lg:w-75 translate-x-0" : "w-0 -translate-x-full"}
        `}
      >
      ...
      {!sidebarVisible && (
        <button
          type="button"
          onClick={toggleSidebar}
          className="fixed bottom-8 left-0 hidden md:flex items-center justify-center w-14 h-12 rounded-r-full bg-primary hover:bg-primary-hover cursor-pointer transition-colors z-50"
